...
assert.equal(
  args.length,
  0,
  "The 'express()' function call should not take any arguments.",
);
```
...
assert.equal(
  value,
  3000,
  "The variable 'port' should be assigned the value 3000.",
);
```
...
assert.isTrue(
  ...
  "Your server should log a message to the console when it starts.",
);
```
...
_Note: Stop the running server with `Ctrl+C` and restart it with `node server.js` for your changes to take effect._
...
Create a global variable `jokes` with the following value:
